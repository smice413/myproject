...
public class MemberAction {
	...
	public String pwd_find_ok(@ModelAttribute MemberBean mem, HttpServletResponse response, Model model)
		...
		if (member == null) {// 값이 없는 경우
			...
			String hostSMTPid = "sungho413@naver.com";
			String hostSMTPpwd = "0000"; // 비밀번호 입력해야함

			// 보내는 사람 EMail, 제목, 내용
			String fromEmail = "sungho413@naver.com";
			String fromName = "관리자";
	...
	public String member_join_ok(@RequestParam("join_profile1") MultipartFile mf, 
		...
		String path = request.getRealPath("upload"); //반드시 upload 폴더를 webapp폴더 하위에 만들어야 한다.
...
//		System.out.println("file1="+file[1]);
	...
	if(filename != ""){	 // 첨부파일이 전송된 경우	
		...
		// 파일 중복문제 해결
		String extension = filename.substring(filename.lastIndexOf("."), filename.length()); //lastIndexOf(".")는 파일명의.을 기준으로 뒤의 글을 length()로 가져옴
		System.out.println("extension:"+extension);
		
		UUID uuid = UUID.randomUUID();
		
		newfilename = uuid.toString() + extension; //파일 중복을 없애기 위해 난수에 확장자를 붙여 새로운 파일변수를 만들어 저장한다.
		...
		if (size > 0) { // 첨부파일이 전송된 경우
			...
			// .transferTo() cos라이브러리가 아니기 때문에 자동업로드가 되지 않아 이 메소드를 써야 업로드 시킬 수 있다.
		}
		...
		return "redirect:member_login.nhn"; //redirect:는 다시 불러오는 것. jsp로 메세지를 뿌리지 않고 컨트롤러에서 또 호출함
	}
	...
	public String member_edit_ok(@RequestParam("join_profile1") MultipartFile mf, 
		...
		String filename = mf.getOriginalFilename();
		int size = (int) mf.getSize();	//첨부파일의 크기	(단위:Byte) getSize()는 long형으로 리턴되기 때문에 형변환 해야함
...
//		System.out.println("file1="+file[1]);
	...
	if(filename != ""){	 // 첨부파일이 전송된 경우		
		...
		// 파일 중복문제 해결
		String extension = filename.substring(filename.lastIndexOf("."), filename.length());//.확장자만 저장됨
		System.out.println("extension:"+extension);
		...
		StringTokenizer st = new StringTokenizer(filename, ".");//확장자가 jpg, gif, png가 아니면 저장 안되게 하기 위해.을 기준으로 분리
		...
		if(size > 100000){				//100KB
			...
			return "member/uploadResult"; // return을 하기 때문에 밑의 다른 실행문이 실행 안됨.
			
		}else if(!file[1].equals("jpg") && //확장자가 jpg, gif,png가 아닌 경우 업로드 안됨
				 !file[1].equals("gif") &&
				 !file[1].equals("png") ){
